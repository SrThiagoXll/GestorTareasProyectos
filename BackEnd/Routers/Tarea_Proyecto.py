...
router = APIRouter(prefix="/TareaProyecto",tags=["Tarea Proyecto"])
...
@router.get("/Obtener_Tarea_Proyecto")
async def Obtener_Tarea_Proyecto(db: Session = Depends(get_db)):

        try:

            # Ejecutar la consulta SQL "ObtenerComentarioTarea" que devuelve ambas tablas
            query = text("EXEC ObtenerTareaProyecto")
