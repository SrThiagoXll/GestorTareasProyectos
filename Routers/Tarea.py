...
from Models.UsuarioSql import Tarea
from sqlalchemy.orm import Session
from Model.Tarea import TareaBase
...
async def obtner_Tarea(proyecto_ID:int, db : Session = Depends(get_db)):          
        tarea = db.query(Tarea).filter(Tarea.Proyecto_ID == proyecto_ID).first()
        if tarea is None:
...
async def crear_Tarea(tarea: TareaBase, db: Session = Depends(get_db)):    
    try:               
        db_tarea = Tarea(Tarea_ID=tarea.Tarea_ID,
...
async def actualizar_Tarea(Tarea_ID: int, tarea: TareaBase, db: Session = Depends(get_db)):
    try:
